# Sequential
rm -f locators/locators.json && pytest tests_generated/test_auto.py -v -s

# Parallel (4 workers)
rm -f locators/locators.json && pytest tests_generated/test_auto.py -v -s -n 4

# Headless parallel
pytest tests_generated/test_auto.py -v -s -n 4 --headless

# Force LLM regeneration after editing userstories.txt
rm -f tests_generated/test_steps.json locators/locators.json
pytest tests_generated/test_auto.py -v -s -n 4

# when make changes in the userstories and locator file
rm -f tests_generated/test_steps.json locators/locators.json
pytest tests_generated/test_auto.py -v -s


# Critical — all three caches must be wiped when switching sites
rm -f tests_generated/test_steps.json
rm -f locators/locators.json
rm -rf tests_generated/_results
...
def _dismiss_overlays(driver):
    ...
    and popup containers that intercept clicks.
...
def _try_click(driver, strategies, label="", use_js=True):
    ...
    Auto-dismisses overlays before each attempt and retries once on
        ...
        for attempt in range(2):
            try:
                ...
                if "intercepted" in err_str and attempt == 0:
                    print(f"[CLICK] Click intercepted, dismissing overlays")
                    _dismiss_overlays(driver)
                    time.sleep(0.5)
                    continue
...
def execute_step(driver, step):
    ...
    # If a strict intent (filter_compare, click_option_with_value,
    # select_option) failed, do NOT fall through to LLM DOM search.
    # LLM fallback for strict intents produces wrong-element clicks.
