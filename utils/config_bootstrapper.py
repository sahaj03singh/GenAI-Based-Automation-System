...
# ──────────────────────────────────────────────────────────────────
def _classify_strategy(intent: str, c) -> str:
    """
    Build the human-facing comment for a single strategy line. Returns
    a string starting with one of:
      ✅          looks correct (verified & not flagged)
      ⚠️ CHECK   verified but suspicious (cross-page wrong pick risk)
      🗑️ DELETE  known noise (consent banners, etc.)
      ⚠️ PROPOSED  multi-match, ambiguous
      👉 FIX     no candidates found at all
    """
    by, val = c.by, c.value
    text = f"{by}={val}".lower()

    # Known noise: consent banners (Funding Choices et al.) and similar.
    noise_markers = ("fc-cta-", "fc-faq-", "fc-vendors", "fc-button",
                     "cookie-consent", "cookieconsent", "onetrust",
                     "didomi", "truste")
    if any(m in text for m in noise_markers):
        return f"🗑️ DELETE: cookie-consent/overlay element — remove from list"

    # Suspicious: a DOM-extracted search-box id appearing for a NON-search intent.
    if c.source == "dom" and intent not in ("search_input", "search_submit"):
        suspicious_ids = ("search_product", "submit_search", "search",
                          "susbscribe_email", "subscribe")
        if any(s == val for s in suspicious_ids):
            return (f"⚠️ CHECK: DOM-extracted prominent element — likely a "
                    f"SEARCH/SUBSCRIBE control, NOT the {intent} target. "
                    f"Verify or delete.")

    if c.tier == VERIFIED:
        return f"✅ [VERIFIED] via {c.source}; matched 1"
    if c.tier == PROPOSED:
        n = c.match_count or "?"
        return (f"⚠️ PROPOSED via {c.source}; matched {n} — verify which "
                f"element is right before keeping")
    # NEEDS_HUMAN
    return f"👉 FIX: via {c.source} did not resolve — supply a manual locator"
...
def emit_config(base_url: str, pages: list[PageResult],
    ...
    """
    Emit a self-documenting draft. Every field a human must touch is
    annotated with a uniform marker so one grep gives the whole to-do
    list:
        grep -nE "👉 FIX|⚠️ CHECK|🗑️ DELETE" site_config_DRAFT.py
    """
    L = []
    add = L.append
    ...
    add("")
    add("═══════════════════════════════════════════════════════════════")
    add(" HOW TO READ THIS DRAFT")
    add("═══════════════════════════════════════════════════════════════")
    add(" Every comment in this file starts with ONE of these markers,")
    add(" telling you exactly what (if anything) you need to do:")
    add("")
    add("   ✅           verified by the bootstrapper, looks correct.")
    add("   ⚠️ PROPOSED   verified but matched >1 element — confirm which.")
    add("   ⚠️ CHECK      verified but likely the WRONG element — review.")
    add("   🗑️ DELETE    known noise (cookie-consent etc.) — remove.")
    add("   👉 FIX       you MUST edit this line before the draft works.")
    add("")
    add(" The bootstrapper does NOT invent intent semantics: phrases,")
    add(" types, strict flags, data_keys, ready-signals and verify-")
    add(" keywords are all 👉 FIX items you supply.")
    add("")
    add(" To see the complete to-do list, run:")
    add("   grep -nE '👉 FIX|⚠️ CHECK|🗑️ DELETE' <this file>")
    add("═══════════════════════════════════════════════════════════════")
    add('"""')
    add("")
    add("SITE_CONFIG = {")
    add(f'    "base_url": "{base_url.rstrip("/")}",  # ✅ '
        f'bootstrap loaded this URL successfully')
    add("")
    ...
    for p in pages:
        add(f'        "{p.name}": "{p.path}",  '
            f'# ✅ loaded -> {p.final_url}')
    add("    },")
    add("")

    # Page ready signals (likely all NEEDS_HUMAN — bootstrapper heuristic is weak here).
    add("    # ── Page ready signals ────────────────────────────")
    add('    "page_ready_signals": {')
    add('        # 👉 FIX: for each page, supply ONE stable element (id or')
    add('        # unique css) whose presence proves the page is ready.')
    add('        # The bootstrapper\'s heuristic cannot reliably pick this.')
    for p in pages:
        if p.ready_signal:
            by, val = p.ready_signal
            add(f'        "{p.name}": ("{by}", "{val}"),  '
                f'# ✅ found a stable signal')
        else:
            add(f'        # "{p.name}": ("?", "?"),  '
                f'# 👉 FIX: no stable signal auto-detected — supply manually')
    add("    },")
    add("")

    # Verify keywords (all proposed).
    add("    # ── Verify keywords ───────────────────────────────")
    add('    "verify_keywords": {')
    add('        # 👉 FIX: for each named page-state, give 3-6 short phrases')
    add('        # that appear ONLY on that page (not generic nav text).')
    add('        # The bootstrapper\'s samples below are GENERIC — replace them.')
    for p in pages:
        if p.visible_text_sample:
            kws = ", ".join(f'"{k}"' for k in p.visible_text_sample[:6])
            add(f'        # "{p.name} loaded": [{kws}],  '
                f'# ⚠️ PROPOSED (generic) — replace with page-unique phrases')
    add("    },")
    add("")
    ...
    for intent, cands in intent_results.items():
        ...
        add(f'            "type": "click_strategies",  '
            f'# 👉 FIX: confirm type '
            f'(click_strategies / click_first_match / form_field / filter_compare)')
        add(f'            "match": [],  '
            f'# 👉 FIX: add the plain-English phrases your tests use')
        add(f'            # 👉 FIX (optional): "strict": True   '
            f'— set if a wrong-element match is worse than no match')
        add(f'            # 👉 FIX (form_field only): "data_key": "<key>"  '
            f'— link to a value in test_data.py')
        add(f'            "strategies": [')
        if not cands:
            add(f'                # 👉 FIX: no candidates found AT ALL — '
                f'supply a manual locator')
        for c in cands:
            comment = _classify_strategy(intent, c)
            add(f'                {c.as_strategy_tuple()},  # {comment}')
        add(f'            ],')
        add(f'        }},')
...
def _emit_filter_compare_intents(add, fp: FilterPattern):
    ...
    pattern_marker = "✅" if fp.tier == VERIFIED else "⚠️ PROPOSED"
    for name, op, matches in intents_spec:
        add(f'        "{name}": {{')
        add(f'            "type": "filter_compare",  # ✅ pattern discovered')
        add(f'            "strict": True,  '
            f'# 👉 FIX (recommended True): prevents silent wrong heals')
        match_lits = ", ".join(f'"{m}"' for m in matches)
        add(f'            "match": [{match_lits}],  '
            f'# 👉 FIX: tweak phrases to match YOUR test cases')
        add(f'            "container_xpath": "{cx}",  '
            f'# {pattern_marker} {fp.note}')
        add(f'            "value_xpath": "{vx}",      '
            f'# {pattern_marker} value path')
        add(f'            "link_xpath": "{lx}",       '
            f'# {pattern_marker} link path')
        add(f'            "extract_pattern": r"{fp.extract_pattern}",  '
            f'# ✅ regex for number inside value (rarely change)')
        add(f'            "operator": "{op}",  # ✅ comparison operator')
        add(f'            "wait_for_url_fragment": "/product",  '
            f'# 👉 FIX: adjust to YOUR product-detail URL pattern '
            f'(e.g. "/product_details/")')
        add(f'        }},')
